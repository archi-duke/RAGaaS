...
"""
            if inverse_search_mode in ["auto", "always"]:
                graph_instruction += "- 역관계가 자동 생성되어 있습니다: 모든 `제자` 엣지에 미러 `스승` 엣지가 존재하며 그 반대도 같습니다. 즉 `(A)-[:제자]->(B)` ⟺ `(B)-[:스승]->(A)`.\n"
                graph_instruction += "- **단일 사실 조회**는 무방향 `(n)-[:REL]-(m)`도 허용됩니다.\n"
                graph_instruction += "- **관계 체인(멀티홉)에서는 무방향 `-[:스승|제자]-`를 절대 쓰지 마세요.** 미러 엣지를 양방향으로 순회해 시작 노드/노이즈를 반환합니다.\n"
                graph_instruction += "- 대신 **방향성 화살표로 한 방향만** 따르세요. 'A의 스승' = `(A)-[:제자]->(t)` 또는 `(A)<-[:스승]-(t)`. 'A의 스승의 스승' = `(A)-[:제자]->(m)-[:제자]->(o)`. (질문 관계가 DB에 반대로 저장돼 있으면 화살표 방향/역관계로 맞추되, 방향은 반드시 명시)\n"
            else:
