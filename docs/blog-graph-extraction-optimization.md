...
def post_process_triples(triples):
    ...
    # 4. [REMOVED] 역관계 자동 생성 (대신 검색 시 양방향 추론 사용)
...
노이즈 관계: 1개 (1.6%)
누락된 관계: 없음 ✅
자동 생성된 역관계: 0개 (검색 시점에 실시간 추론하므로 저장 불필요) ✅
```

---

## 새로운 아키텍처: 물리적 생성에서 검색 시점 추론으로

처음에는 `A -> 선배 -> B`가 저장될 때 `B -> 후배 -> A`를 자동으로 생성하여 저장하는 방식을 사용했습니다. 하지만 이는 다음과 같은 한계를 가졌습니다:
1. **데이터 중복**: 데이터 양이 2배로 늘어남.
2. **동기화 문제**: 수정/삭제 시 두 트리플을 모두 관리해야 함.
3. **유연성 부족**: 사전에 정의되지 않은 관계(예: "학교 후배")는 역관계를 만들지 못함.

### 해결: 양방향 검색(Bidirectional Search)

이제는 저장은 원본 하나만 하고, **검색 시점에 화살표의 방향을 가리지 않고 훑는 방식**을 채택했습니다.
...
이 방식은 **"검색 단계에서의 방향성 제약을 없애버리는 것"**이 핵심입니다. 논리적 추론(`A가 후배니까 B는 선배구나`)은 LLM이 답변 생성 단계에서 문맥을 보고 완벽하게 수행합니다. 이로써 데이터 정합성은 유지하면서 검색 재현율은 극대화할 수 있었습니다.
