...
async def update_kb_prompt():
    ...
    # Update test2 KB
    # Note: Beanie/Mongo often uses _id as the primary key string if defined that way in model, 
    # OR it uses _id separately. The python script above showed `_id: '...'`.
    # Let's try matching by _id
    result = await collection.update_one(
        {"_id": KB_ID},
        {"$set": {"sparql_prompt_template": CORRECT_PROMPT}}
    )
    
    print(f"Matched: {result.matched_count}, Modified: {result.modified_count}")
    
    # Verify
    new_doc = await collection.find_one({"_id": KB_ID})
    if new_doc:
        print("Updated KB Prompt Preview:", new_doc.get("sparql_prompt_template", "")[:100])
    else:
        print("KB Found Failed")
