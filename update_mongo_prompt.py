...
async def update_mongo_prompt():
    client = AsyncIOMotorClient(MONGO_URI)
    db = client[DB_NAME]
    collection = db["prompts"]
    
    # Update sparql_generation_prompt
    result = await collection.update_one(
        {"name": "sparql_generation_prompt"},
        {"$set": {"content": CORRECT_PROMPT, "type": "sparql"}},
        upsert=True
    )
    ...
    # Verify
    new_doc = await collection.find_one({"name": "sparql_generation_prompt"})
    print("Updated Content Preview:", new_doc["content"][:100])

if __name__ == "__main__":
    asyncio.run(update_mongo_prompt())
